...
class LSTMPooling(nn.Module):
    def __init__(self, backbone_config, pooling_config, is_lstm=True):
        ...
        if self.is_lstm:
            self.lstm = nn.LSTM(
                self.hidden_size, self.hidden_lstm_size, bidirectional=self.bidirectional, batch_first=True
            )
        else:
            self.lstm = nn.GRU(
                self.hidden_size, self.hidden_lstm_size, bidirectional=self.bidirectional, batch_first=True
...
class WeightedLayerPooling(nn.Module):
    def __init__(self, backbone_config, pooling_config):
        ...
        self.layer_weights = (
            pooling_config.layer_weights
            if pooling_config.layer_weights is not None
            else nn.Parameter(torch.tensor([1] * (self.num_hidden_layers + 1 - self.layer_start), dtype=torch.float))
        )
...
class GeMText(nn.Module):
    def __init__(self, hidden_size, p, dim, eps):
